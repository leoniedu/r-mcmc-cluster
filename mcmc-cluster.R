...
cluster.jags.model <- function(cl,file, data = sys.frame(sys.parent()), inits,n.adapt = 1000,model.name="mnow") {
    ##max should be 4 chains because jags currently uses only 4 Pseudo-RNGs
    ...
    tev <- paste(model.name," <- jags.model(file, data, inits, n.chains=1,n.adapt)")
    ## exports arguments
...
clusterMCMC.snow <- function (cl,f, ...) {
    ...
    fargs <- list(...)
    lapply(names(fargs),function(x) with(fargs,clusterExport2(cl,x,get(x))))
